<template>
  <div class="callback">
    ...
    >
      ...
      <circle class="fill --primary" cx="352.52" cy="290.99" r="84.65" />
      <polygon
        class="fill --base"
...
export default {
  ...
  mounted() {
    ...
    if (this.logged) {
      setTimeout(() => {
        fn.loginUser(fn.userData)
          .then(() => fn.$router.push('/'))
          .catch(err => fn.$errorGlobalHandler(err))
      }, 500)
    }
...
.callback {
  ...
  @apply bg-base;
  @apply font-raleway text-2xl text-secondary text-center;
